...
def process_timelapse_with_mask(data_dir: Path, mask_file: Path, output_csv: Path):
    ...
    if not all_image_files:
        ...
        return
    print(f"Found {len(all_image_files)} images to process in {data_dir}")

    all_frames_data = []
    for frame_index, image_path in enumerate(tqdm(all_image_files, desc="Analyzing Frames")):
...
# --- Configuration ---
parser = argparse.ArgumentParser(description="Apply a master mask to a folder of images.")
...
if args.input_dir:
    ...
    # As with run_cellpose, we make a best-guess for paths when overriding config
    OUTPUT_DIR = DATA_ROOT / "data/processed" / f"{EXPERIMENT_NAME}_Fiji"
    MASK_FILE = DATA_ROOT / "data/processed/cellpose" / EXPERIMENT_NAME / f"{EXPERIMENT_NAME}_master_mask.png"
    OUTPUT_CSV = OUTPUT_DIR / f"{EXPERIMENT_NAME}_Results.csv"
else:
    ...
    MASK_FILE = DATA_ROOT / "data/processed/cellpose" / f"{EXPERIMENT_NAME}_{MICROSCOPY_TYPE}" / MASK_FILENAME

    OUTPUT_CSV = OUTPUT_DIR / f"{EXPERIMENT_NAME}_{MICROSCOPY_TYPE}_Results.csv"

OUTPUT_DIR.mkdir(exist_ok=True)


# --- Run Processing ---
print(f"Input image directory: {DATA_DIR}")
print(f"Using master mask: {MASK_FILE}")
